...
public sealed class UpdateSpecializationCommandHandler
    : IRequestHandler<UpdateSpecializationCommand, ErrorOr<Success>>
{
    ...
    public async Task<ErrorOr<Success>> Handle(UpdateSpecializationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await _specializationRepository.ExistsWithNameAsync(name, request.Id, cancellationToken))
        ...
        var deactivated = specialization.Update(name, request.Status, updatedBy, now);
